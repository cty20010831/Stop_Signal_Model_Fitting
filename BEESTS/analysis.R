...
print(paste("Reading analysis scripts from: ", path_analysisDir))
summary.statistics = as.logical(args[2])
print(paste("Calculate summary statistics: ", args[2]))
posterior.distributions = as.logical(args[3])
print(paste("Plot posterior distributions: ", args[3]))
mcmc.chains = as.logical(args[4])
print(paste("Plot mcmc chains: ", args[4]))
posterior.predictors = as.logical(args[5])
print(paste("Conduct posterior predictive check: ", args[5]))

# Read original data
dat = read.csv(sub("_(?!.*_).*", "", path_analysisDir, perl = TRUE), 
               head=TRUE,sep=",")
print("Successfully read original data.")
ncol = ncol(dat)

# Read analysis description
path__analysisDescription <- file.path(path_analysisDir, "analysis.txt")
