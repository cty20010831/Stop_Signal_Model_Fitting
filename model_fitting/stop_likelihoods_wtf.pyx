...
cdef extern from "gsl/gsl_integration.h":
    ...
    int gsl_integration_qag(gsl_function * f,
                            double a, double b,
    ...
    int GSL_INTEG_GAUSS41  # Integration key constant
...
cdef inline double ExGauss_pdf(double value, double mu, double sigma, double tau):
    """ ExGaussian log pdf"""
    cdef double z
    if tau > 0.05*sigma:
...
cdef inline double ExGauss_cdf(double value, double mu, double sigma, double tau):
...
########################
cdef double eval_cexgauss(double x, void * params) noexcept nogil:
    ...
    with gil:
        p = exp(ExGauss_cdf(x, imu_go, isigma_go, itau_go)) * \
                 exp(ExGauss_pdf(x - issd, imu_stop, isigma_stop, itau_stop)) * (1 - ip_tf)
